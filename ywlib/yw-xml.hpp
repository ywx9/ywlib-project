...
class xml {
  static constexpr bool is_digit(const char32_t c) noexcept {
    return '0' <= c && c <= '9';
  }
  static constexpr bool is_alphabet(const char32_t c) noexcept {
    return ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z');
  }
  static constexpr bool is_encname(const char32_t c) noexcept {
    return is_alphabet(c) || is_digit(c) || c == '-' || c == '_' || c == '.';
  }
  static constexpr bool is_character(const char32_t c) noexcept {
    return c == 0x9 || c == 0xa || c == 0xd || (0x20 <= c && c <= 0xd7ff) || (0xe000 <= c && (c & 0xfffe) != 0xfffe);
  }
  static constexpr bool is_whitespace(const char32_t c) noexcept {
    return c == 0x20 || c == 0x9 || c == 0xd || c == 0xa;
  }
  static constexpr bool is_namestartchar(const char32_t c) noexcept {
  ...
  static constexpr bool is_namechar(const char32_t c) noexcept {
...
private:
  ...
  ustrview encoding;
  bool standalone{};
  ustrview prolog, root;
  array<processing_instruction> instructions;
  void analyze() {
    ustrview sv = raw;
    auto result = analyze_xmldecl(sv);
    result = analyze_miscs(sv);
    // analyze_doctypedecl
  }
  ustrview get_name(const ustrview& sv) {
    if (sv.empty() || !is_namestartchar(sv.front())) return {};
    auto it = std::ranges::find_if_not(sv, is_namechar);
    return ustrview(sv.begin(), it);
  }
  int analyze_miscs(ustrview& sv) {
    if (std::ranges::starts_with(sv, U"<!--"sv)) {
      ...
      instructions.push_back({target, ustrview{sv.begin(), sr.begin()}});
...
public:
  xml() = default;
  xml(ustr&& Text) : raw(mv(Text)) { analyze(); }
  xml(stringable auto&& Text) : raw(unicode<uchar>(Text)) { analyze(); }
  xml(const path& File) : raw(convert(File.read())) { analyze(); }
};
}
...
constexpr bool is_pubidchar(const uchar c) noexcept {
...
constexpr ustrview get_versionnum(ustrview& s, const ustrview& Document) {
  constexpr auto is_digit = [](char32_t c) { return c >= U'0' && c <= U'9'; };
...
//   constexpr auto is_space = [](const char32_t c) noexcept { return (c == 0x20 || c == 0x9 || c == 0xd || c == 0xa); };
//   auto it                 = s.begin();
...
// constexpr bool is_namestartchar(const char32_t c) noexcept {
//   return ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z') || c == ':' || c == '_' ||
//          (0xc0 <= c && c <= 0x2ff && c != 0xd7 && c != 0xf7) ||
//          (0x370 <= c && c <= 0x1fff && c != 0x37e) || c == 0x200c || c == 0x200d ||
...
// constexpr bool is_namechar(const char32_t c) noexcept {
//   return ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z') || ('0' <= c && c <= ':') || c == '.' || c == '_' ||
//          (c == 0xb7) || (0xc0 <= c && c <= 0x2ff && c != 0xd7 && c != 0xf7) ||
//          (0x300 <= c && c <= 0x1fff && c != 0x37e) || c == 0x200c || c == 0x200d || c == 0x203f || c == 0x2040 ||
//          (0x2070 <= c && c <= 0x218f) || (0x2c00 <= c && c <= 0x2fef) ||
...
//   constexpr auto is_digit = [](const char32_t c) noexcept { return '0' <= c && c <= '9'; };
//   constexpr auto is_hex   = [](const char32_t c) noexcept { return is_digit(c) || ('a' <= c && c <= 'f') || ('A' <= c && c <= 'F'); };
...
// // constexpr bool is_namestartchar(const char32_t u) noexcept {
...
// // constexpr bool is_namechar(const char32_t u) noexcept {
...
// //   auto fr2 = std::ranges::find_if(ustrview(fr, Str.end()), [](const char32_t u) { return !is_namechar(u); });
// //   Name     = ustr(fr, fr2);
